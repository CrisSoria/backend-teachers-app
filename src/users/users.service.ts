...
export class UsersService {
  ...
  async update(id: string, updateUserDto: UpdateUserDto): Promise<User> {
    try {
      ...
      // Error de duplicado
      if (error.code === 11000) {
        throw new UserEmailAlreadyExistsException(updateUserDto.email|| 'no se proporciono email');
      }
  ...
  async existsByEmail(email: string): Promise<boolean> {
    try {
      ...
      this.logger.error(`Error al verificar existencia de email: ${error.message}`, error.stack);
      throw new HttpException(
        {
          statusCode: HttpStatus.INTERNAL_SERVER_ERROR,
